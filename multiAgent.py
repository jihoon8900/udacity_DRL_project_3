...
class MultiAgent():
  def __init__(self, env, state_size, action_size, random_seed=15, device=device):
    ...
    for _ in range(self.agent_num):
      self.agents.append(
          Agent(self.state_size, self.action_size, LR_ACTOR, LR_CRITIC,
                WEIGHT_DECAY, EPSILON, EPSILON_DECAY, self.device, self.agent_num)
      )
    ...
    self.memory = ReplayBuffer(
        self.action_size, BUFFER_SIZE, BATCH_SIZE, random_seed)
  ...
  def step(self, state, action, reward, next_state, done, t):
    ...
    full_state, full_action, full_reward, full_next_state, full_done = self.transform_to_full(
        state, action, reward, next_state, done)

    # save full experiecne to memory
    self.memory.add(full_state, full_action, full_reward,
                    full_next_state, full_done)
  ...
  def learn(self, experiences, agent_idx, gamma):
    ...
    with torch.no_grad():
      actions_next = torch.cat([self.agents[idx].actor_target(full_next_states[:, idx*self.state_size:(
          idx+1)*self.state_size]) for idx in range(self.agent_num)], dim=-1)  # (batch_size, action_size*agent_num)
      actions_next += torch.clip(torch.normal(mean=0., std=0.2,
                                 size=actions_next.shape), -1.0, 1.0).to(self.device)  # TD3
      actions_next = torch.clip(actions_next, -1.0, 1.0)
      Q_targets_next = torch.min(self.agents[agent_idx].critic_target_1(
          full_next_states, actions_next), self.agents[agent_idx].critic_target_2(full_next_states, actions_next))  # TD3

    # Compute Q targets for current states (y_i)
    Q_targets = local_rewards + (gamma * Q_targets_next * (1 - local_dones))

    # Compute critic loss
    Q_expected_1 = self.agents[agent_idx].critic_local_1(
        full_states, full_actions)
    Q_expected_2 = self.agents[agent_idx].critic_local_2(
        full_states, full_actions)
    ...
    torch.nn.utils.clip_grad_norm_(
        self.agents[agent_idx].critic_local_1.parameters(), 1.0)
    torch.nn.utils.clip_grad_norm_(
        self.agents[agent_idx].critic_local_2.parameters(), 1.0)
    ...
    if self.iterations[agent_idx] % self.policy_delay == 0:
      ...
      actor_loss = - \
          self.agents[agent_idx].critic_local_1(
              full_states, actions_pred).mean()
      # Minimize the loss
      self.agents[agent_idx].actor_optimizer.zero_grad()
      actor_loss.backward()
      torch.nn.utils.clip_grad_norm_(
          self.agents[agent_idx].actor_local.parameters(), 1.0)
      self.agents[agent_idx].actor_optimizer.step()

      # ----------------------- update target networks ----------------------- #
      self.soft_update(self.agents[agent_idx].critic_local_1,
                       self.agents[agent_idx].critic_target_1, TAU)
      self.soft_update(self.agents[agent_idx].critic_local_2,
                       self.agents[agent_idx].critic_target_2, TAU)
      self.soft_update(self.agents[agent_idx].actor_local,
                       self.agents[agent_idx].actor_target, TAU)

    self.agents[agent_idx].epsilon -= self.agents[agent_idx].epsilon_decay

  def act(self, states, add_noise=True):
    actions = np.concatenate([agent.act(np.expand_dims(
        state, axis=0), add_noise) for agent, state in zip(self.agents, states)], axis=0)
    return actions

  def noise_reset(self):
    for idx in range(self.agent_num):
      self.agents[idx].noise.reset()
  ...
  def train(self, n_episodes=1800, max_t=3000):
    ...
    for i_episode in range(1, n_episodes+1):
      env_info = self.env.reset(train_mode=True)[
          self.brain_name]  # reset the environment
      # get the current state
      state = env_info.vector_observations[:, -8:]
      ...
      for t in range(max_t):
        ...
        next_state, reward, done = env_info.vector_observations[:, -
                                                                8:], env_info.rewards, env_info.local_done
      ...
      self.iterations = np.zeros(
          (self.agent_num,))               # reset iteration
      # save most recent score
      max_scores_window.append(np.max(score))
      # save most recent score
      self.scores.append(np.max(score))
      print('\rEpisode {}\tAverage Score : {:.4f} \t eps : {:.3f}'.format(
          i_episode, np.mean(max_scores_window), self.agents[0].epsilon), end="")
      if i_episode % 50 == 0:
        print('\rEpisode {}\tAverage Score : {:.4f} \t eps : {:.3f}'.format(
            i_episode, np.mean(max_scores_window), self.agents[0].epsilon))
      if np.mean(max_scores_window) >= 0.5:
        print('\nEnvironment solved in {:d} episodes!\tAverage Score: {:.34}'.format(
            i_episode-100, np.mean(max_scores_window)))
        torch.save(self.agents[0].actor_local.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_1_TD3_actor.pth')
        torch.save(self.agents[0].critic_local_1.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_1_TD3_critic_1.pth')
        torch.save(self.agents[0].critic_local_2.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_1_TD3_critic_2.pth')
        torch.save(self.agents[1].actor_local.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_2_TD3_actor.pth')
        torch.save(self.agents[1].critic_local_1.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_2_TD3_critic_1.pth')
        torch.save(self.agents[1].critic_local_2.state_dict(
        ), os.getcwd() + '/saved_model/saved_agent_2_TD3_critic_2.pth')
        break
    return self.scores
